  ...
  return (
    ...
    >
      ...
      <TiltCard maxTilt={4} glareOpacity={0.08}>
        <div
          className={`relative z-10 rounded-2xl overflow-hidden transition-all duration-500 ${hovered ? "animated-border" : ""}`}
          style={{
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-5">
                  <motion.span
                    ...
                    animate={hovered ? { letterSpacing: "0.2em" } : { letterSpacing: "0.1em" }}
                ...
                <div className="relative mb-3">
                  <motion.span
                    ...
                    animate={hovered ? { opacity: 0.7, scale: 1.05 } : { opacity: 0.4, scale: 1 }}
                    transition={{ duration: 0.4 }}
                  >
                ...
                {metrics.length > 0 && (
                  <div className="flex gap-4 mb-5 pb-5 border-b border-border/30">
                    {metrics.map(([key, val], mi) => (
                      <motion.div
                        key={key}
                        className="text-center"
                        initial={{ opacity: 0, y: 8 }}
                        animate={inView ? { opacity: 1, y: 0 } : {}}
                        transition={{ delay: index * 0.08 + 0.3 + mi * 0.1 }}
                      >
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {project.techStack.slice(0, 5).map((t, ti) => (
                    <motion.span
                      key={t}
                      initial={{ opacity: 0, scale: 0.8 }}
                      animate={inView ? { opacity: 1, scale: 1 } : {}}
                      transition={{ delay: index * 0.08 + 0.4 + ti * 0.05 }}
    ...
    <section className="section">
      <div className="container">
        ...
        >
          ...
          <Link
            href="/projects"
            className="self-start sm:self-auto group flex items-center gap-2 px-4 py-2 rounded-xl text-sm text-muted-foreground border border-border/40 hover:border-primary/40 hover:text-primary glass transition-all gradient-underline"
          >
            All projects
        ...
        <div className="flex flex-col gap-6">
          {featured.map((project, i) => (
            <div key={project.id}>
              ...
              {i < featured.length - 1 && <SectionDivider variant="dots" />}
            </div>
          ))}
        </div>
