...
function StatCard({ stat, index }: { stat: (typeof stats)[0]; index: number }) {
  ...
  return (
    ...
    >
      <div className="text-2xl sm:text-3xl font-bold tabular-nums leading-none mb-1"
        style={{ background: "linear-gradient(135deg,#a78bfa,#818cf8,#67e8f9)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>
        {inView ? (
          <CountUp start={0} end={numeric} duration={2.2} decimals={decimals}
            suffix={hasPlus ? "+" + stat.suffix : stat.suffix} useEasing />
        ) : (
          <span>{stat.value}{stat.suffix}</span>
        )}
      </div>
      <div className="text-[11px] text-white/55 leading-tight">{stat.label}</div>
    </motion.div>
  );
}
...
export function Hero() {
  ...
  const tiltY = useTransform(springX, [-0.5, 0.5], [-3, 3]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-[1]">
        {/* Left-heavy dark vignette so text is readable */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/65 to-black/10" />
        {/* Bottom fade into next section */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/30" />
        {/* Subtle colour tint — matches brand purple */}
        <div className="absolute inset-0 bg-gradient-to-br from-violet-950/40 via-transparent to-cyan-950/20" />
        {/* Film grain noise */}
        <div
      ...
      >
        {/* Glass panel wrapping all content — desktop left half */}
        <motion.div
        ...
        >
          ...
          >
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full text-sm text-white/70 mb-6"
            ...
            <motion.h1
              ...
              className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.06] mb-4"
            >
            ...
            <motion.p
              ...
              className="text-base text-white/60 leading-relaxed mb-8 max-w-lg"
            >
              {personal.tagline}
            </motion.p>
            ...
            >
              <Link
                href="/projects"
                className="group relative inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-sm text-white overflow-hidden shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all"
                style={{
              ...
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-sm text-white/90 hover:text-white hover:-translate-y-0.5 active:translate-y-0 transition-all"
              ...
              <a
                href="/resume"
                className="inline-flex items-center gap-2 px-4 py-3 rounded-xl text-sm text-white/50 hover:text-white/80 transition-colors"
              >
                <Download size={14} />
                Resume
              </a>
            ...
            >
              {[
                ...
                <a
                  ...
                  className="group flex items-center gap-2 px-4 py-2 rounded-lg text-sm text-white/55 hover:text-white transition-all"
